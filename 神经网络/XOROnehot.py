# -*- coding: utf-8 -*-
"""
Created on Mon Mar 21 15:44:17 2022
XOR onehot
@author: Xinnze
"""
...
y_pred = tf.nn.softmax(tf.add(tf.matmul(layer_1, weights['h2']), biases['h2']))

loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(y_pred), 1))
loss1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=layer_1))
train_step = tf.train.AdamOptimizer(learning_rate).minimize(loss1)
...
for i in range(1000):
    ...
    if i % 100 == 0:
        print('loss = ', "{:.9f}".format(sess.run(loss1, feed_dict={x: X, y: Y})))
